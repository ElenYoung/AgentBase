...
| `get_current_holdings(curr_date, feedback=None)` | 返回当前日期的持仓 |
...
**约束条件：**
- 所有权重必须非负
- 权重总和不超过 1（允许持有现金，需要用货币基金ETF的code来表示持有现金）
...
| `get_current_holdings(curr_date, feedback=None)` | 获取持仓（抽象方法） |
| `get_daily_holdings(start_date, end_date)` | 获取日期范围内的持仓 |
